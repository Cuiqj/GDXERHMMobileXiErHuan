    ...
    switch (tableView.tag) {
            ...
            //路产损坏清单
        {
            ...
            if (indexPath.section==0) {
                ...
                if (deformation.price.floatValue < 0.0) {
                    cell.assetPriceLabel.text = UNDETERMINED;
                }else{
                ...
                if (self.viewLocal == 0) {
                    ...
                    if ([deformation.unit rangeOfString:@"米"].location != NSNotFound) {
                        ...
                        if (deformation.quantity.doubleValue > deformation.quantity.integerValue) {
                            cell.assetQuantityLabel.text=[NSString stringWithFormat:@"%.2f%@",deformation.quantity.doubleValue,deformation.unit];
                        }else{
                ...
                if(deformation.total_price.doubleValue < 0.0){
                    cell.assetTotalAmountLabel.text = UNDETERMINED;
                }else{
            ...
            if (indexPath.section==1) {
                ...
                for (CaseDeformation *t in self.deformList) {
                ...
                if (flag == TRUE) {
                    cell.detailTextLabel.text= UNDETERMINED;
                }else{
...
-(void)addDeformationForRoadAsset:(RoadAssetPrice *)aRoadAsset andQuantity:(double)quantity withRemark:(NSString *)aRemark{
    if (![self.caseID isEmpty]) {
        if (self.citizenList.count>0) {
            ...
            CaseDeformation *newDeformation = [CaseDeformation newDataObjectWithEntityName:@"CaseDeformation"];
            newDeformation.proveinfo_id = self.caseID;
            ...
            if (aRoadAsset.price.doubleValue < 0) {
                newDeformation.total_price = [NSNumber numberWithDouble:-1.0];
            }else{
...
-(void)reloadInfo{
    ...
    if (self.citizenList.count>0) {
        //刚进入的时候
        [self tableView:self.citizenPickerView didSelectRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:0]];
    }
