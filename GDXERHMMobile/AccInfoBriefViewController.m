...
- (void)viewDidLoad
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(updateParkingNodeInfo) name:@"toUpdateParkingNodeInfo" object:nil];
}

-(void)viewWillAppear:(BOOL)animated{
//    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(deleteParkingNumber:) name:@"DeleteCitizen" object:nil];
//    [self updateParkingNodeInfo];
}
...
// 更新停驶车辆的停驶地点
- (void)updateParkingNodeInfo{
    ParkingNode *parkingNode = [[ParkingNode parkingNodesForCase:self.caseID] lastObject];
...
- (IBAction)parkingChanged:(UISwitch *)sender {
    ...
    [UIView transitionWithView:self.view 
                    ...
                    completion:nil];
    NSArray * citizenArr = [Citizen allCitizenNameForCase:self.caseID];
    
    for (Citizen * citizen in citizenArr) {
        self.textParkingCarName.text = citizen.automobile_number;
    }
...
-(void)saveParkingNodeForCase:(NSString *)caseID{
    ...
    // 不能全部删除，否则无法保存原有的ParkingNode | zhenlintie 2014-04-16
//    [ParkingNode deleteAllParkingNodeForCase:caseID];
    ...
    if (self.swithIsParking.isOn) {
        if (![self.textParkingCarName.text isEmpty]) {
            ...
            for (NSString *carNumber in numberArray){
                // 把汽车牌号存在了citizen_name ...
                if (![carNumber isEmpty]) {
                    ParkingNode *parkingNode;
                    parkingNode = [ParkingNode parkingNodesForCase:self.caseID carNumber:carNumber];
                    if (!parkingNode){
                    ...
                    parkingNode.citizen_name=carNumber;
