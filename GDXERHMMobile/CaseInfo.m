...
+(NSArray*)caseInfoByCaseMark2:(NSString *)case_mark2 withCaseMark3:(NSString *)case_mark3{
    ...
    NSEntityDescription *entity=[NSEntityDescription entityForName:@"CaseInfo" inManagedObjectContext:context];
    NSFetchRequest *fetchRequest=[[NSFetchRequest alloc] init];
    NSPredicate *predicate=[NSPredicate predicateWithFormat:@"case_mark3 == %@ && case_mark2 == %@",case_mark3,case_mark2];
    fetchRequest.predicate=predicate;
    fetchRequest.entity=entity;
    NSArray *fetchResult=[context executeFetchRequest:fetchRequest error:nil];
    return fetchResult;
...
+ (NSInteger) maxCaseMark3:(NSString *)year{
    NSManagedObjectContext *context=[[AppDelegate App] managedObjectContext];
    NSFetchRequest *fetchRequest = [[NSFetchRequest alloc] initWithEntityName:@"CaseInfo"];
    NSPredicate *predicate=[NSPredicate predicateWithFormat:@"case_mark2 == %@",year];
    fetchRequest.predicate = predicate;
    NSError *error = nil;
    NSArray *caseInfos = [context executeFetchRequest:fetchRequest error:&error];
    if(!error){
         NSSortDescriptor *aSortDescriptor  = [NSSortDescriptor sortDescriptorWithKey:@"case_mark3.intValue" ascending:YES];
        caseInfos = [NSMutableArray arrayWithArray:[caseInfos sortedArrayUsingDescriptors:[NSArray arrayWithObject:aSortDescriptor]]];
        CaseInfo *caseinfo = (CaseInfo *)(caseInfos.lastObject);
        return [caseinfo.case_mark3 integerValue];
    }
    return 0;
}
...
- (NSString *)fullCaseMarkAfterK:(BOOL)isAfterK
{
    ...
    NSString *t = [self.side stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]];
    if([t isEqual:@"南行"]  || [t isEqual:@"北行"]){
        t = [NSString stringWithFormat:@"（%@）",t ];
    }else{
        t = @"";
    }
    if (isAfterK) {
        return [NSString stringWithFormat:@"K%@+%@%@",self.station_start_km,self.station_start_m,t];
    } else {
        return [NSString stringWithFormat:@"%@+%@%@",self.station_start_km,self.station_start_m,t];
    }
...
- (NSString *)full_happen_place{
    ...
    if([self.side rangeOfString:@"收费站" options:NSBackwardsSearch].location == 2 || [self.place rangeOfString:@"匝道" options:NSBackwardsSearch].location == 1){
...
- (NSString *)full_happen_place2{
    ...
    if([self.side rangeOfString:@"收费站" options:NSBackwardsSearch].location == 2 || [self.place rangeOfString:@"匝道" options:NSBackwardsSearch].location == 1){
        ...
        return [NSString stringWithFormat:@"%@%@K%@+%@处", roadName, self.side, [self station_start_km], [self station_start_m]];
    }
}
- (NSString *)happeningPlace{
    return [NSString stringWithFormat:@"%@%@", self.side, self.place];
}
@end
