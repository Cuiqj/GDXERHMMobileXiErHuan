...
- (void)viewDidLoad
{
    ...
    if (![self.caseID isEmpty]) {
        ...
        if (self.caseServiceReceipt == nil) {
            ...
            [self generateDefaultInfo2:self.caseServiceReceipt];
        }
        [self pageLoadInfo];
    }
    [super viewDidLoad];
...
- (void)pageLoadInfo{
    ...
    CaseProveInfo *proveInfo;
    if(self.citizen.automobile_number != nil && ![self.citizen.automobile_number isEmpty]){
        proveInfo = [CaseProveInfo proveInfoForCase:self.caseID citizenName:self.citizen.automobile_number];
    }else{
        proveInfo = [CaseProveInfo proveInfoForCase:self.caseID];
    }
    ...
    self.textremark.text = [NSStringNilIsBad(self.caseServiceReceipt.remark) emptyFor:@"无"];
...
- (void)generateDefaultInfo:(CaseServiceReceipt *)caseServiceReceipt{
    ...
    caseServiceReceipt.incepter_name=self.citizen.party;
...
- (void)generateDefaultInfo2:(CaseServiceReceipt *)caseServiceReceipt{
    CaseInfo *caseInfo = [CaseInfo caseInfoForID:self.caseID];
    if(self.citizen == nil){
        self.citizen = [Citizen citizenForCitizenName:nil nexus:@"当事人" case:self.caseID];
    }
...
- (id)dataForPDFTemplate
{
    ...
    CaseProveInfo *caseProve;
    if(self.citizen.automobile_number != nil && ![self.citizen.automobile_number isEmpty]){
        caseProve = [CaseProveInfo proveInfoForCase:self.caseID citizenName:self.citizen.automobile_number];
    }else{
        caseProve = [CaseProveInfo proveInfoForCase:self.caseID];
    }
    if (caseProve) {
    ...
    NSString *comment = [NSStringNilIsBad(self.caseServiceReceipt.remark) emptyFor:@"无"];
    id data = @{
                @"case": caseData,
                @"recipient": recipient,
...
- (void)setParty:(Citizen *)citizen{
    ...
    if (![self.caseID isEmpty]) {
        self.caseServiceReceipt = [CaseServiceReceipt caseServiceReceiptForCase:self.caseID incepterName:citizen.party];
        if (self.caseServiceReceipt == nil) {
            self.caseServiceReceipt = [CaseServiceReceipt newCaseServiceReceiptForCase:self.caseID];
            [self generateDefaultInfo2:self.caseServiceReceipt];
        }
