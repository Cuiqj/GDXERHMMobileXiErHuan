...
- (void)generateDefaultInfo:(CaseProveInfo *)caseProveInfo{
    ...
    if ([caseProveInfo.event_desc length] <= 0) {
        //citizen_name是车牌号
        if(caseProveInfo.citizen_name != nil && ![caseProveInfo.citizen_name isEmpty]){
            caseProveInfo.event_desc = [CaseProveInfo generateEventDescForCase:self.caseID automobileNumber:caseProveInfo.citizen_name];
        }else{
