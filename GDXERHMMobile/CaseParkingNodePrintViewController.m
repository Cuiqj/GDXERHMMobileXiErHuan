...
// add by zhenlintie 2014-04-16 保存 停驶地点
- (void)saveParkingInfo
{
    [self.labelSendDate setText:@""];
    CaseInfo *caseInfo = [CaseInfo caseInfoForID:self.caseID];
	NSMutableString *stationStart = [self.textFieldStationStart.text mutableCopy];
	if(stationStart != nil && ![stationStart isEmpty]){
		NSRange range = [stationStart rangeOfString:@"（"];
		if(range.length > 0){
			NSRange endRange = [stationStart rangeOfString:@"）" options:NSBackwardsSearch];
			if(endRange.length > 0){
				NSRange subStringRanege = NSMakeRange(range.location+1, endRange.location - range.location-1);
				caseInfo.side = [stationStart substringWithRange:subStringRanege];
			}
		}
	}
    if (caseInfo) {
...
- (void)pageLoadInfo
{
    ...
    if (caseInfo) {
        ...
        NSArray * parkings = [ParkingNode parkingNodesForCase:self.caseID];
        NSArray * automobileArr = [parkings valueForKeyPath:@"@unionOfObjects.citizen_name"];
        
        for (NSString * str in automobileArr) {
            
            self.textFieldAutomobileNumber.text = str;
        }
        
        [self loadParkingInfoWithAutomobileNumber:self.textFieldAutomobileNumber.text];
    }
}
...
- (void)pageSaveInfo{
    ...
    [self saveParkingInfo];
    
    [[AppDelegate App] saveContext];
    
    [[NSNotificationCenter defaultCenter] postNotificationName:@"toUpdateParkingNodeInfo" object:nil];
    [[NSNotificationCenter defaultCenter] postNotificationName:@"toUpdateCaseInfo" object:nil];
}
...
- (id)dataForPDFTemplate
{
    ...
    if (caseInfo) {
        ...
        NSString *dateString2 = NSStringNilIsBad(self.textFieldHappenDate.text);
        NSArray *dateComponents2 = [dateString2 componentsSeparatedByCharactersInSet:[NSCharacterSet characterSetWithCharactersInString:@"年月日"]];
        if (dateComponents.count > 2) {
            happenDate = @{
                           @"year": dateComponents2[0],
                           @"month": dateComponents2[1],
                           @"day": dateComponents2[2],
                           };
        }
