...
@synthesize textInsuranceCompany = _textInsuranceCompany;
@synthesize textInsuranceNO = _textInsuranceNO;
...
- (void)viewDidUnload
{
    ...
    [self setTextInsuranceCompany:nil];
    [self setTextInsuranceNO:nil];
    [super viewDidUnload];
...
-(void)saveCitizenInfoForCase:(NSString *)caseID{
    ...
    if (![aAutoNumber isEmpty] && ![self.textParty.text isEmpty]) {
        ...
        citizen.insurance_company = _textInsuranceCompany.text == nil? @"" : _textInsuranceCompany.text;
        citizen.insurance_no = _textInsuranceNO.text == nil? @"" : _textInsuranceNO.text;
...
-(void)loadCitizenInfoForCase:(NSString *)caseID autoNumber:(NSString *)aAutoNumber andNexus:(NSString *)nexus{
    ...
    //add by 李晓明 加载数据前需要清除旧数据
    {
        ...
        _textInsuranceNO.text = @"";
        _textInsuranceCompany.text = @"";
    }
    ...
    if (![aAutoNumber isEmpty]) {
        ...
        if (citizen){
            ...
            _textInsuranceCompany.text = citizen.insurance_company? citizen.insurance_company : @"";
            _textInsuranceNO.text = citizen.insurance_no? citizen.insurance_no : @"";
        } else {
