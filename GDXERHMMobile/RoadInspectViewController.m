...
- (void) createRecodeByCaseID:(NSString *)caseID{
    ...
    [dateFormatter setDateFormat:DATE_FORMAT_HH_MM_COLON];
    NSString *timeString=[dateFormatter stringFromDate:inspectionRecord.start_time];
    NSMutableString *remark=[[NSMutableString alloc] initWithFormat:@"%@巡至%@往%@方向K%@+%@处时，发现%@驾驶%@在%@发生交通事故，",timeString, [RoadSegment roadNameFromSegment:caseInfo.roadsegment_id], caseInfo.side, [caseInfo station_start_km], [caseInfo station_start_m], citizen.party, [citizen automobileName], caseInfo.place];
    if ([caseInfo.fleshwound_sum intValue]==0 && [caseInfo.badwound_sum intValue]==0 && [caseInfo.death_sum intValue]==0) {
        ...
        [remark appendFormat:@"受伤%@人，死亡%@人，", caseInfo.badwound_sum, caseInfo.death_sum];
    }
    ...
    if (deformArray.count>0) {
        ...
        [remark appendFormat:@"结果如下：%@。",deformsString];
    } else {
